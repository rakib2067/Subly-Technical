...
export const MediumItem: React.FC<{item: Medium}> = ({item}) => {
  ...
  if(item.status=='ready'){
    overlay=<div className={`overlay ${!show? 'hide':''}`} >
    ...
    <span className='languages'>{item.languages.length} languages</span>
  </div>
  }
  return (
    <li className='medium' onMouseOut={()=>setShow(false)} onMouseOver={()=> setShow(true)}>
